                            ...
                            return (
                                <div className={styles.experienceCard} key={key} data-aos="fade-up">
                                    <Box borderWidth="1px" borderRadius="md" overflow="hidden">
                                        <div style={{ background: currentTheme.contrastText, height: '150px', color: currentTheme.contrastText }}></div>
                                        <div className={styles.experienceCardContent}>
                                            <h1>{exp.company}</h1>
                                            <div className={styles.avatar}>
                                                <Avatar size="xl" name={exp.company} >{exp.companylogo}</Avatar>
                                            </div>
                                            <div style={{ position: 'relative', top: '20px' }}>
                                                <h2>{exp.position}</h2>
                                                <h3>{exp.description}</h3>
                                            </div>
                                            <h4 style={{ color: currentTheme.contrastText }}>{exp.time}</h4>
                                        </div>
                                    </Box>
                                </div>
                            )
